...
#
# --backup-only:
#    - Non-interactive "Old Server" mode: let user select site(s), create local backups and exit.
#
...
check_tools() {
  local missing=0
  for c in mysqldump mysql tar gzip rsync stat; do
...
do_old_server_backup() {
  ...
  for wp in "${SELECTED_WP_PATHS[@]}"; do
    ...
    if [[ -f /root/.my.cnf ]]; then
      ...
      mysqldump -u root -p "$db" | gzip > "$DB_FILE" || {
        err "DB backup failed for $db"
        rm -f "$DB_FILE"
        continue
      }
    ...
    parent="$(dirname "$wp")"
    base="$(basename "$wp")"
    tar -czf "$FILES_FILE" -C "$parent" "$base" || {
...
do_new_server_restore() {
  ...
  tar -xzf "$latest_files" -C "$TMP_DIR" || {
...
main() {
  ...
  case "${1-}" in
    --backup-only)
      # Old server backup mode for selected sites
      do_old_server_backup --non-interactive
      ;;
    "" )
      echo
      echo "Select migration mode:"
      echo "  1) Old Server: Push sites from THIS server to another server (LOCAL backups)"
    ...
    * )
      ...
      echo "  wp-migrate-local.sh --backup-only      # backup selected sites locally (for migration)"
      exit 1
      ;;
  esac
}
